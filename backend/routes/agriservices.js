...
const fs = require("fs");
router.get(
  "/agri/getFileData/:startDate/:endDate/:type/:country",
  (req, res) => {
    ...
    let parser = parse({ columns: true }, async (err, records) => {
      ...
      let recs = [];
      console.log(
        `header key ${countryName} and result len is ${result.length}`
      );
      await result.map((rec) => {
        ...
        for (let i = 0; i < keys.length; i++) {
          ...
          if (year != null && year >= startDate && year <= endDate) {
            ...
            let rdata =
              rec[year] == null || isNaN(rec[year]) || rec[year].length == 0
                ? 0
                : parseFloat(rec[year]);
    ...
    fs.createReadStream(__dirname + "/../csv/agri/" + type + ".csv").pipe(
      parser
    );
  }
);
